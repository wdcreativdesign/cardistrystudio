...
import {
  ImageUp, RotateCcw, Play, Pause, Sun, RefreshCcw, CreditCard, Pipette, Eye, Layers, Download, Move,
} from 'lucide-react'
  ...
  return (
    <div className="space-y-2.5">
      <div className="flex items-center justify-between">
        ...
        <span className="text-[11px] text-black/35 font-mono tabular-nums w-14 text-right">
          {display}
        </span>
      </div>
      <Slider
...
function BgSection({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  function handleHexCommit(raw: string) {
    ...
    if (/^#[0-9a-fA-F]{6}$/.test(full)) { onChange(full); setHex(full) }
  }
    ...
    <div className="space-y-3">
      <div className="flex gap-2">
        {BG_PRESETS.map((p) => (
          <button
            key={p.value}
            onClick={() => { onChange(p.value); if (p.value !== 'transparent') setHex(p.value) }}
            className={cn(
...
export function ControlPanel({ settings, onChange, onReset, onExport }: ControlPanelProps) {
  ...
  return (
    <aside className="flex flex-col w-[278px] min-w-[278px] h-screen bg-white border-l border-black/[0.07] overflow-y-auto shadow-[-8px_0_24px_-8px_rgba(0,0,0,0.06)]">
      ...
      {/* ── Tabs ── */}
      <div className="flex gap-0.5 p-2 border-b border-black/[0.07] sticky top-0 bg-white z-10">
        <button
          onClick={() => setTab('create')}
          className={cn(
            'flex-1 text-[12px] font-medium py-2 rounded-xl transition-all',
            tab === 'create'
